...
export default function GalleryPage({ category, onBack }: { category: Category; onBack: () => void }) {
  ...
  return (
    <div className="pt-20">
      <div className="relative h-[35vh] min-h-[250px] flex items-end overflow-hidden bg-[#080808]">
        <img
          src={category.cover}
          alt={category.title}
        ...
        {category.id === 'reels' && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative">
              <motion.div
                className="absolute -inset-6 rounded-full border border-[#c9a96e]/20"
                animate={{ opacity: [0.3, 0.05, 0.3], scale: [1, 1.2, 1] }}
                transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
              />
              <div className="w-20 h-20 flex items-center justify-center rounded-full bg-[#c9a96e]/80 shadow-2xl shadow-[#c9a96e]/30">
        ...
        <div className="relative z-10 w-full px-6 lg:px-10 pb-8 max-w-7xl mx-auto">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-1.5 font-sans text-[0.5rem] tracking-[0.3em] uppercase text-white/40 hover:text-[#c9a96e] transition-colors duration-300 mb-4 font-medium"
          >
            <ChevronLeft size={12} />
            All Categories
          </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {category.items.map((item, i) => (
            <motion.div
              ...
              className="group relative overflow-hidden cursor-pointer bg-[#0a0a0a] aspect-[4/5]"
            ...
            >
              {item.type === 'video' ? (
                <>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="relative">
                      <motion.div
                        className="absolute -inset-3 rounded-full border border-[#c9a96e]/30"
                        animate={{ opacity: [0.4, 0.1, 0.4], scale: [1, 1.15, 1] }}
                        transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
                      />
                      <div className="relative w-16 h-16 flex items-center justify-center rounded-full bg-[#c9a96e]/90 shadow-lg shadow-[#c9a96e]/20 transition-transform duration-300 group-hover:scale-110">
              ...
              <div className="absolute inset-0 shadow-[inset_0_0_0_0_rgba(201,169,110,0)] group-hover:shadow-[inset_0_0_0_1px_rgba(201,169,110,0.3)] transition-all duration-500" />
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <AnimatePresence>
        {lightboxItem && (
          ...
          >
            <button
              onClick={closeLightbox}
              className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center border border-[#1a1a1a] text-[#c9a96e60] hover:text-[#c9a96e] hover:border-[#c9a96e30] transition-all z-10"
              aria-label="Close"
            >
              <X size={14} />
            </button>
            <button
              onClick={prev}
              className="absolute left-4 md:left-8 w-10 h-10 flex items-center justify-center border border-[#1a1a1a] text-[#c9a96e60] hover:text-[#c9a96e] hover:border-[#c9a96e30] transition-all z-10"
              aria-label="Previous"
            >
              <ArrowLeft size={14} />
            </button>
            <button
              onClick={next}
              className="absolute right-4 md:right-8 w-10 h-10 flex items-center justify-center border border-[#1a1a1a] text-[#c9a96e60] hover:text-[#c9a96e] hover:border-[#c9a96e30] transition-all z-10"
              aria-label="Next"
            >
              <ArrowRight size={14} />
            </button>
            ...
            >
              ...
              <div className="flex items-center justify-between w-full mt-5">
                <div>
                  <p className="font-sans text-[0.48rem] tracking-[0.5em] text-[#c9a96e60] uppercase mb-1 font-medium">{category.title}</p>
